...
export async function repository(userId:string|null,admin=false){
 ...
 const local=localDemo();const client=local?null:admin?supabaseAdmin():await supabaseServer();
 ...
 async createPlace(input:PlaceInput,first:KeywordInput){const id=randomUUID(),now=new Date().toISOString();if(!local){const {data,error}=await client!.rpc('create_place_with_keyword',{p_place:input,p_keyword:first});if(error)throw new Error(errorMessage(error));return data as string;}const db=localDb();db.exec('BEGIN');try{await insert('places',{id,user_id:userId!,...input,created_at:now,updated_at:now});await insert('place_keywords',{id:randomUUID(),place_id:id,...first,created_at:now,updated_at:now});db.exec('COMMIT');return id;}catch(e){db.exec('ROLLBACK');throw e;}},
